...
- A static-analysis wrapper around Bandit, detect-secrets, pip-audit, native binary inspection (PE/ELF triage), and Jupyter notebook code-cell projection.
...
- It does not execute notebooks; `.ipynb` code cells are copied to temporary Python source projections for static analysis.
...
`inspect_binary.py` is a required file in the v1.6.0 release package for native PE/ELF binary inspection. If it is missing, the scanner still runs, but PE/ELF binary triage is skipped and the log contains `Binary inspection helper not found`.
...
Supported scan inputs include Python package archives (`.whl`, `.egg`, `.zip`, `.tar.gz`, `.tgz`), loose Python source (`.py`, `.pyw`), and Jupyter notebooks (`.ipynb`). Notebook saved outputs and attachments are surfaced as parser findings because they are visible payloads but are not executable Python code-cell input.
