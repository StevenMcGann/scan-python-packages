...
Fixtures are deterministic by design: generated random values use fixed seeds, ZIP and TAR members use fixed timestamps, archive members are added in sorted order, and native-binary stubs are fixed 64-byte byte strings. Running the generator twice without source changes should produce byte-identical corpus contents.
